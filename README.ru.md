...
- **Чат ExecAI предустановлен и живёт в правой панели** — `Ctrl+L` фокусирует его. Чаты — вкладками вверху панели: клик переключает, «×» закрывает, перетаскивание меняет порядок. Ссылки и пути к файлам в ответах кликабельны, у каждого блока кода есть кнопка копирования.
- **11 источников моделей** — бэкенд ExecAI, твои ключи Z.ai / Kimi Code / Moonshot / Anthropic / OpenAI / OpenRouter или полностью локально через Ollama. Переключение на лету, одна общая история. — бэкенд ExecAI, твои подписки Z.ai / Kimi / Anthropic / OpenAI или полностью локально через Ollama. Переключение на лету, одна общая история.
